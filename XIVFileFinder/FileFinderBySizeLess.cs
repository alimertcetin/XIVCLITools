...
{
    public class FileFinderBySizeLess
    {
        public int Run(string[] args)
        {
            ...
            var matchedFiles = FindFiles(path, p => FileFindOptions.ByLengthLess(p, maxFileSizeByte));

            if (matchedFiles.Count <= 0)
            {
                Logger.Log("No file found less than " + maxFileSizeMB + "MB");
                return 0;
            }
            Logger.Log("Files less than " + maxFileSizeMB);
            ...
            {
                var folderPath = Path.Combine(path, "Files Under " + maxFileSizeMB);
